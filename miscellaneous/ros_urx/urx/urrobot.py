...
class URRobot(object):
    ...
    def movej(self, joints, acc=0.1, vel=0.05, wait=True, relative=False, threshold=None, radius=0.0):
        """
        move in joint space
        """
        if relative:
    ...
    def movexs_mod(self, command, pose_list, acc=0.01, vel=0.01, radius=0.01, wait=True, threshold=None):
        """
        Concatenate several movex commands and applies a blending radius
        pose_list is a list of pose.
        This method is usefull since any new command from python
        ...
        for idx, pose in enumerate(pose_list):
            if idx == (len(pose_list) - 1):
                radius = 0
            prog += self._format_move(command, pose, acc, vel, radius) + "\n"
        prog += end
        self.send_program(prog)
        if wait:
            self._wait_for_move(target=pose_list[-1], threshold=threshold)
